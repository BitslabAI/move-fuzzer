                        ...
                        for line in lines:
                            if (file,str(i)) in warning_map.keys():
                                ...
                                if w["type"] == "unused_use":
                                    ...
                                    lines[i-1] = lines[i-1][:index] + "0x999"+item_1 + lines[i-1][index+len(w["item_1"]):]
                                    # lines[i] = lines[i].replace(w["info"].split("'")[1],"0x999")
                                elif w["type"] == "unused_assignment":
                                    index = lines[i-1].find(w["item_1"])
                                    lines[i-1] = lines[i-1][:index] + "0x999" + lines[i-1][index+len(w["item_1"]):]
